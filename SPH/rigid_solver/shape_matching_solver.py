    ...
    def compute_temp_center_of_mass(self):
        ...
        for p_i in range(self.container.particle_num[None]):
            if (
                ...
                self.rigid_body_temp_centers_of_mass[object_id] += (
                    self.container.V0
    ...
    def solve_constraints(self):
        ...
        for p_i in range(self.container.particle_num[None]):
            if (
                ...
                self.rigid_body_temp_rotation_matrices[object_id] += (
                    self.container.V0 * self.container.particle_densities[p_i] * p.outer_product(q)
                )
        ...
        for p_i in range(self.container.particle_num[None]):
            if (
                ...
                object_id = self.container.particle_object_ids[p_i]
                goal = self.rigid_body_temp_centers_of_mass[object_id] + self.rigid_body_temp_rotation_matrices[
                    object_id
                ] @ (
                    self.container.rigid_particle_original_positions[p_i]
                    - self.container.rigid_body_original_centers_of_mass[object_id]
