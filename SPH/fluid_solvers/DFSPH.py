...
class DFSPHSolver(BaseSolver):
    ...
    def compute_density_derivative_task(self, p_i, p_j, ret: ti.template()):
        ...
        ret.density_adv += self.container.particle_rest_volumes[p_j] * ti.math.dot(
            v_i - v_j, self.kernel_gradient(pos_i - pos_j)
        )
    ...
    def compute_kappa_v(self):
        ...
        for idx_i in range(self.container.particle_num[None]):
            if self.container.particle_materials[idx_i] == self.container.material_fluid:
                self.container.particle_dfsph_kappa_v[idx_i] = (
                    self.container.particle_densities_derivatives[idx_i] * self.container.particle_dfsph_alphas[idx_i]
                )
    ...
    def correct_divergence_task(self, p_i, p_j, ret: ti.template()):
        if self.container.particle_materials[p_j] == self.container.material_fluid:
            ...
            if ti.abs(k_sum) > self.m_eps * self.dt[None]:
                ...
                ret.dv -= (
                    grad_p_j
                    * (k_i / self.container.particle_densities[p_i] + k_j / self.container.particle_densities[p_j])
                    * self.density_0
                )
    ...
    def compute_kappa(self):
        ...
        for idx_i in range(self.container.particle_num[None]):
            if self.container.particle_materials[idx_i] == self.container.material_fluid:
                self.container.particle_dfsph_kappa[idx_i] = (
                    (self.container.particle_densities_star[idx_i] - 1.0)
                    * self.container.particle_dfsph_alphas[idx_i]
                    * delta_t_inv
                )
