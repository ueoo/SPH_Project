...
class BaseSolver:
    def __init__(self, container: BaseContainer):
        ...
        if self.viscosity_method == "implicit":
            ...
            self.original_velocity = ti.Vector.field(
                self.container.dim, dtype=ti.f32, shape=self.container.particle_max_num
            )
            ...
            self.cg_diagnol_ii_inv = ti.Matrix.field(
                self.container.dim, self.container.dim, dtype=ti.f32, shape=self.container.particle_max_num
            )
        ...
        if self.container.particle_materials[p_j] == self.container.material_fluid:
            ...
            if R2 > diameter2:
                ret -= (
                    ...
                    / self.container.particle_masses[p_i]
                    * self.container.particle_masses[p_j]
                    * R
                    * self.kernel_W(R.norm())
                )
            else:
                ret -= (
                    self.surface_tension
                    ...
                    * self.container.particle_masses[p_j]
                    * R
                    * self.kernel_W(ti.Vector([self.container.particle_diameter, 0.0, 0.0]).norm())
                )
    ...
    def add_viscosity_force_to_rigid_task(self, p_i, p_j, ret: ti.template()):
        if (
            self.container.particle_materials[p_j] == self.container.material_rigid
            and self.container.particle_is_dynamic[p_j]
        ):
            pos_i = self.container.particle_positions[p_i]
            pos_j = self.container.particle_positions[p_j]
            # Compute the viscosity force contribution
    ...
    def _renew_rigid_particle_state(self):
        ...
        for p_i in range(self.container.particle_num[None]):
            if (
                ...
                and self.container.particle_is_dynamic[p_i]
            ):
                ...
                if self.container.rigid_body_is_dynamic[object_id]:
                    ...
                    q = (
                        ...
                        - self.container.rigid_body_original_centers_of_mass[object_id]
                    )
                    p = rotation @ q
    ...
    def renew_rigid_particle_state(self):
        ...
        if self.cfg.get_cfg("exportObj"):
            for obj_i in range(self.container.object_num[None]):
                if (
                    self.container.rigid_body_is_dynamic[obj_i]
                    and self.container.object_materials[obj_i] == self.container.material_rigid
                ):
                    ...
                    ret = (
                        ...
                        @ (
                            ...
                            - self.container.object_collection[obj_i]["restCenterOfMass"]
                        ).T
                    )
