...
class BaseContainer:
    def __init__(self, config: SimConfig, GGUI=False):
        ...
        self.particle_max_num = (
            ...
            + (
                self.compute_box_particle_num(
                    self.domain_box_start,
                    self.domain_box_size,
        ...
        print(
            f"Fluid particle num: {self.fluid_particle_num}, Rigid body particle num: {self.rigid_body_particle_num}"
        )
        ...
        self.object_num[None] = (
            num_fluid_object + num_rigid_object + (1 if self.add_domain_box else 0)
        )  # add 1 for domain box object
        ...
        self.rigid_particle_original_positions_buffer = ti.Vector.field(
            self.dim, dtype=float, shape=self.particle_max_num
        )
    ...
    def reorder_particles(self):
        ...
        for i in range(self.particle_num[None]):
            ...
            self.grid_ids_new[p_i] = (
                ti.atomic_sub(self.grid_num_particles_temp[self.grid_ids[p_i]], 1) - 1 + base_offset
            )
    ...
    def load_rigid_body(self, rigid_body, pitch=None):
        ...
        # ! if the center of mass is not exactly the base frame center, this will lead to error
        ...
        # # TODO: make it parallel or pre compute and store
    ...
    def load_fluid_body(self, rigid_body, pitch=None):
        ...
        print(
            f"processing {len(new_positions)} points to decide whether they are inside the mesh. This might take a while."
        )
        inside = [False for _ in range(len(new_positions))]
    ...
    def compute_box_particle_num(self, lower_corner, cube_size, thickness, space=None):
        ...
        for i in range(self.dim):
            mask = mask | (
                (new_positions[:, i] <= lower_corner[i] + thickness)
